...
export function getBookCoverUrl(
  book: Pick<Book, '_id' | 'coverUrl' | 'coverImageToken' | 'coverImageSyncedAt'>,
): string {
...
class ApiClient {
  ...
  private async request<T>(
    endpoint: string,
    options: RequestInit & { timeoutMs?: number; suppressErrorToast?: boolean } = {},
  ): Promise<T> {
    ...
    const timeout =
      typeof window !== 'undefined' ? window.setTimeout(() => controller.abort(), timeoutDuration) : undefined;
    try {
      ...
      if (!response.ok) {
        const errorData =
          typeof payload === 'object' && payload
            ...
            : {};
        const message =
          errorData.error || (typeof payload === 'string' && payload.trim()) || `Request failed (${response.status}).`;
        const error = new ApiError(message, response.status, errorData.code, errorData.details);
        if (typeof window !== 'undefined' && response.status !== 401 && !suppressErrorToast)
          window.dispatchEvent(new CustomEvent('novels-library:api-error', { detail: { message, variant: 'error' } }));
        throw error;
      }
      return payload as T;
    } catch (error) {
  ...
  async listAdminUsers(query: { search?: string; page?: number; limit?: number } = {}) {
    ...
    return this.request<{ users: AdminUser[]; total: number; page: number; limit: number; totalPages: number }>(
      `/admin/users${suffix}`,
    );
  }
  ...
  async enableAdminResource(id: string, isEnabled: boolean) {
    return this.request<{ resource: AdminResource }>(`/admin/resources/${id}/enable`, {
      method: 'PUT',
  ...
  async listAdminAuditLogs(query: { page?: number; limit?: number } = {}) {
    ...
    if (query.limit !== undefined) params.set('limit', String(query.limit));
    const suffix = params.toString() ? `?${params.toString()}` : '';
    return this.request<{ logs: AdminAuditLog[]; total: number; page: number; limit: number; totalPages: number }>(
  ...
  async getNotifications(
    unreadOnly?: boolean,
  ): Promise<{ notifications: Notification[]; unreadCount: number; pagination: Pagination }> {
    const query = unreadOnly ? '?unreadOnly=true' : '';
    return this.request<{ notifications: Notification[]; unreadCount: number; pagination: Pagination }>(
      `/notifications${query}`,
    );
  }
  ...
  async triggerScrape(
    bookId: string,
    type: JobType,
  ): Promise<{ success: boolean; message: string; job: BackgroundJob }> {
  ...
  async runScrapeNow(
    ...
    return this.request<{ success: boolean; message: string; result: unknown; book: Book; job: BackgroundJob }>(

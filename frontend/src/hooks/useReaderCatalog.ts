...
export function useReaderCatalog({
  ...
  const missingChapterTitle =
    currentCatalogItem?.title || `${isRawReader ? 'Raw chapter' : 'Chapter'} ${chapterNumber}`;

  const previousChapterNumber =
    currentCatalogIndex > 0 ? catalogItems[currentCatalogIndex - 1].chapterNumber : chapterNumber - 1;

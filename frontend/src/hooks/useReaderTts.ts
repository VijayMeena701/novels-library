...
const effectivePlaybackEngineName = allowLocalTts ? playbackEngineName : playbackEngineName === "local" ? "system" : playbackEngineName;
...
const hasAutoCorrectedVoiceRef = useRef(false);
useEffect(() => {
if (!voices.length || !onVoiceChange) return;
if (effectiveVoiceURI !== voiceURI) {
if (hasAutoCorrectedVoiceRef.current) return;
hasAutoCorrectedVoiceRef.current = true;
onVoiceChange(effectiveVoiceURI);
} else {
hasAutoCorrectedVoiceRef.current = false;
}
}, [voices, voiceURI, effectiveVoiceURI, onVoiceChange]);
...
highlightSpeechBlock(location.blockIndex, { skipScroll: true });

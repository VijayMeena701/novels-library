...
export default function PublicBookDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canManageCatalog =
    hasCapability(CAPABILITY.BOOKS_MANAGE) ||
    hasCapability(CAPABILITY.BOOKS_CREATE) ||
    hasCapability(CAPABILITY.BOOKS_UPDATE);
  ...
  const translatedArchivePercent = translatedChapterTotal
    ? Math.min(100, Math.round((chapters.length / translatedChapterTotal) * 100))
    : 0;
      ...
      {activeTab === 'read' && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
          ...
          <BookDetailsSidebar
            book={book}
            chaptersCount={chapters.length}
            sortedItemsCount={translatedCatalogItems.length}
          />
        </div>
      )}

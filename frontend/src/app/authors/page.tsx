    ...
    <div className={cn("mx-auto w-full max-w-[1520px] px-5 pt-6 pb-12", "flex flex-col gap-5")}>
      <div className="flex items-end justify-between gap-4 py-1">
        <div>
          ...
          <p className="text-secondary max-w-[720px]">Browse books by pen name, real name, and aliases.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,250px))] gap-3.5">
          {authors.map((author) => (
            <Link key={author._id} href={`/authors/${author._id}`} className="no-underline">
              <div className="flex h-full min-h-[150px] flex-col gap-3 rounded-lg border border-default bg-card p-5 shadow-elevation-2 transition hover:border-hover hover:bg-card-hover hover:shadow-elevation-4">
                <h3 className="text-[1.1rem]">{author.displayName}</h3>
                {author.realName && <p className="text-[0.9rem] text-secondary">Real name: {author.realName}</p>}
                {author.alternativeNames.length > 0 && (
                  <p className="text-[0.82rem] text-muted">{author.alternativeNames.slice(0, 3).join(', ')}</p>
                )}
                <span className="mt-auto text-[0.85rem] text-secondary">

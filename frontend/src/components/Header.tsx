...
const privateLinks = [{ href: "/profile", label: "Profile", match: (pathname: string) => pathname.startsWith("/profile") }];
...
export default function Header() {
	...
	const links = user ? [...publicLinks, ...privateLinks, ...(user.role === "admin" ? adminLinks : [])] : publicLinks;
		...
		<header className="sticky top-0 z-[100] border-b border-border bg-[rgba(255,253,248,0.88)] shadow-[0_2px_18px_rgba(48,39,28,0.04)] backdrop-blur-[18px]">
			<div className="mx-auto flex min-h-[62px] w-full max-w-[1520px] items-center justify-between gap-4 px-5 py-2.5 max-[860px]:items-start max-[860px]:flex-col">
				<div className="flex w-full items-center justify-between gap-3">
					...
					<button
						type="button"
						className="hidden rounded-md border border-border bg-[rgba(255,253,248,0.88)] px-3 py-1.5 text-[0.82rem] font-bold text-copy max-[860px]:inline-flex"
				...
				<nav
					className={cn(
						"flex flex-wrap items-center justify-end gap-1 max-[860px]:w-full max-[860px]:justify-start",
						!isMobileMenuOpen && "max-[860px]:hidden",
					)}
				>
					{links.map((link) => {

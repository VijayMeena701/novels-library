...
import type {
  CreatePronunciationRulePayload,
  PronunciationRule,
  UpdatePronunciationRulePayload,
} from '../../utils/api';
    ...
    <button
      ...
      className={cn(
        ...
        checked
          ...
          : 'border-default bg-surface text-muted hover:text-primary',
      )}
    >
      {label}
    </button>
...
export function PronunciationRulesModal(props: PronunciationRulesModalProps) {
  ...
  const renderRuleRow = (rule: PronunciationRule) => (
    <li
      key={rule._id}
      className={cn(
        'flex flex-col gap-1.5 rounded-md border border-default bg-surface p-2.5 transition',
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-reader-overlay p-3" onClick={handleClose}>
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-muted">
              Customize how the reader speaks words or phrases in{' '}
              <strong className="text-secondary">{bookTitle}</strong>. Leave the replacement empty to skip a word
              entirely.
            </p>
          </div>
          <button
        ...
        <form
          ...
          className="flex flex-col gap-2.5 rounded-md border border-default bg-surface-raised p-3"
        >
        ...
        {loading ? (
          ...
          <p className="text-xs text-muted">
            No pronunciation rules yet. Add one above to customize this book&apos;s TTS.
          </p>
        ) : (

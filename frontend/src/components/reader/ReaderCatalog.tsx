...
export function ReaderCatalog(props: ReaderCatalogProps) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      (item) => item.title.toLowerCase().includes(query) || item.chapterNumber.toString().includes(query),
    );
  }, [items, search]);
                ...
                >
                  <button
                    ...
                    className={cn(
                      ...
                      item.chapterNumber === currentChapterNumber && 'border-reader-accent/50 bg-reader-surface',
                    )}
                    onClick={() => {
                      onClose();
                  ...
                  >
                    ...
                    <small className="text-[0.68rem] text-reader-muted">
                      {item.archived ? 'Archived' : 'Indexed only'}
                    </small>
                  </button>
                </div>
              );

...
export function MoreTab(props: ReaderBottomToolbarProps) {
	...
	return (
		<div className="flex flex-col gap-5">
			<div>
				<h2 className="text-sm font-semibold text-reader-paragraph">Quiet extras</h2>
				<p className="mt-1 text-xs leading-relaxed text-reader-muted">Keep supporting actions nearby without taking focus from the text.</p>
			</div>
			<div className="grid grid-cols-2 gap-2">
			{props.sourceUrl && (
				<a href={props.sourceUrl} target="_blank" rel="noreferrer" className="contents">
					<DockButton label="Source">
						<span className="text-[0.68rem] text-reader-muted">Original page</span>
					</DockButton>
				</a>
			)}
			<DockButton onClick={props.onScrollToTop} label="Top">
				<span className="text-[0.68rem] text-reader-muted">Return to title</span>
			</DockButton>
			<Link href={props.isLoggedIn ? `/books/${props.bookId}` : getLoginHref(pathname)} className="contents">
				<DockButton label={props.isLoggedIn ? "Profile Details" : "Login"}>
					<span className="text-[0.68rem] text-reader-muted">

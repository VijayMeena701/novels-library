    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-reader-muted">
          Choose how chapters are opened and which source the reader should use.
        </p>
      </div>
      ...
      {enabledOptions.length > 1 && (
        <Field label="Reader type" labelClassName="normal-case font-medium tracking-wide text-reader-muted">
          <div
            className={cn(
              'overflow-hidden rounded-lg border border-reader',
              enabledOptions.length === 2 ? 'grid grid-cols-2' : 'grid grid-cols-3',
            )}
          >

...
export function BookAdminConsole({
  ...
  const translatedPipelineSections: PipelineSection[] =
    canScrape || canImport
      ? [
          {
            ...
            actions: [
              {
                ...
                disabled:
                  !canScrape ||
                  !(book?.translatedChaptersList || []).length ||
                  Boolean(queueing) ||
                  activeJobTypes.has('scrape_chapters'),
  ...
  const rawPipelineSections: PipelineSection[] =
    canScrape || canImport
      ? [
          {
            key: 'raw-indexing',
            ...
            actions: [
              {
                ...
                tone: 'raw',
                disabled:
                  !canScrape || !book?.rawSourceUrl || Boolean(queueing) || activeJobTypes.has('scrape_raw_metadata'),
            ...
            actions: [
              {
                ...
                disabled:
                  !canScrape ||
                  (book.rawChaptersList || []).length === 0 ||
                  Boolean(queueing) ||
                  activeJobTypes.has('scrape_raw_chapters'),
  ...
  return (
    <Card className="p-[1.1rem] bg-gradient-to-b from-surface to-surface-raised border-default flex flex-col gap-6 shadow-elevation-1">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="block text-[10px] font-black tracking-wider text-muted uppercase mb-1">
        ...
        <div className="p-4 border border-default rounded-md bg-surface-raised flex flex-col justify-between">
          ...
          <div className="w-full h-1.5 bg-surface-raised rounded-full overflow-hidden mt-2">
            <div
              ...
              style={{ width: `${translatedArchivePercent}%` }}
            ></div>
          </div>
        </div>
      </div>
      ...
      {adminMessage && (
        <div className="p-3.5 border border-info/20 rounded-md bg-info/10 text-primary text-sm">{adminMessage}</div>
      )}
            ...
            {recentJobs.map((job) => (
              ...
              >
                ...
                <div className="grid gap-0.5 min-w-0">
                  <strong className="text-xs font-extrabold capitalize text-primary">
                ...
                <span className="md:ml-auto">
                  <Badge variant={getJobBadgeVariant(job.status)} className="text-[9px]">

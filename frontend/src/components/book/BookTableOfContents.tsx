					...
					{chapters.length === 0 ? (
						<span>
							...
							{book.sourceUrl && (
								<>
									{" "}
									<a href={book.sourceUrl} target="_blank" rel="noreferrer" className="text-accent hover:underline font-bold">
										Open source page
									</a>
									.
								</>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[520px] overflow-y-auto pr-1">
						{sortedItems.slice(0, 120).map((chapterItem) => (
							...
							>
								...
								{!chapterItem.archived && <small className="text-[9px] font-semibold text-muted uppercase">Indexed only</small>}
							</Link>
						))}
					</div>

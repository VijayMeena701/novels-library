...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const showToast = useCallback(
    (input: ToastInput) => {
      const toast: Toast = {
        ...
        title:
          input.title ||
          (input.variant === 'success' ? 'Success' : input.variant === 'info' ? 'Notice' : 'Something went wrong'),
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div
        className="pointer-events-none fixed inset-x-4 top-4 z-[100] flex flex-col items-end gap-3 sm:left-auto sm:w-[min(100%-2rem,24rem)]"
        role="region"

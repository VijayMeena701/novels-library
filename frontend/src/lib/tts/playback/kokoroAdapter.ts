...
export interface LocalTTSModelAdapter {
	...
	synthesize(text: string, options: LocalTTSModelSynthesizeOptions): Promise<{ data: Float32Array; sampleRate: number }>;
}
...
export class KokoroAdapter implements LocalTTSModelAdapter {
	...
	private handleMessage = (event: MessageEvent<KokoroWorkerResponse>): void => {
		...
		if (response.type === "synthesized") {
			debugLog("KokoroAdapter: received synthesized audio", { byteLength: response.audio.byteLength, sampleRate: response.sampleRate });
		}
	...
	async synthesize(text: string, options: LocalTTSModelSynthesizeOptions): Promise<{ data: Float32Array; sampleRate: number }> {
		...
		if (data.length > 0) {
			...
			debugLog("KokoroAdapter: Float32Array stats", { length: data.length, min, max, zeroCount, first20: Array.from(data.slice(0, 20)) });
		}

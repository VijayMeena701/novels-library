...
export class PlaybackManager extends EventTarget {
  ...
  speakKeepAlive(text: string): void {
    ...
    if (
      ...
      typeof (this.engine as { speakKeepAlive?: (text: string, config: PlaybackConfig) => void }).speakKeepAlive ===
        'function'
    ) {
      (this.engine as { speakKeepAlive: (text: string, config: PlaybackConfig) => void }).speakKeepAlive(
        text,
        this.config,
      );
    } else {

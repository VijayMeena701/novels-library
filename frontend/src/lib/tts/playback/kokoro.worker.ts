...
async function tryLoadKokoro(
	KokoroTTS: unknown,
	dtype: string,
	device: string,
): Promise<KokoroInstance | null> {
	try {
		const factory = KokoroTTS as { from_pretrained: (modelId: string, options: { dtype: string; device: string }) => Promise<unknown> };
	...
	initializePromise = (async () => {
		...
		if (gpu && !webgpuFailed) {
			...
			if (adapter) {
				...
				debugLog("WebGPU adapter:", info.vendor, info.architecture, info.description, "powerPreference:", powerPreference, "supportsF16:", adapter.features.has("shader-f16"));
...
async function processRequest(request: KokoroWorkerRequest, isRetry = false): Promise<void> {
	try {
		...
		const audioBuffer =
			samples.buffer instanceof ArrayBuffer && samples.byteOffset === 0 && samples.byteLength === samples.buffer.byteLength
				? samples.buffer
				: (samples.slice().buffer as ArrayBuffer);
		debugLog(
			"Kokoro worker transfer:",
			JSON.stringify({ audioBufferByteLength: audioBuffer.byteLength, samplesLength: samples.length, sampleRate: result.sampling_rate ?? result.sample_rate ?? 24000 }),
		);
		post(
			{
				id: request.id,

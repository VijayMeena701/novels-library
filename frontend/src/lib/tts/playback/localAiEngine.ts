...
function logAudioContextStats(ctx: AudioContext | null, label: string): void {
  if (!ctx) return;
  debugLog(
    `${label}: state=${ctx.state}, sampleRate=${ctx.sampleRate}, outputLatency=${ctx.outputLatency}, baseLatency=${ctx.baseLatency}, currentTime=${ctx.currentTime}`,
  );
}
...
export class LocalAIEngine implements PlaybackEngine {
  ...
  constructor(adapter?: LocalTTSModelAdapter) {
    ...
    if (typeof window !== 'undefined') {
      ...
      win.__kokoroDiagnostic = {
        bypassNext: () => {
          this.diagnosticBypassNext = true;
        },
  ...
  private async synthesizeToBuffer(
    ...
    if (data.length > 0) {
      ...
      debugLog(
        `LocalAIEngine: PCM stats min=${min.toExponential(3)} max=${max.toExponential(3)} zeroCount=${zeroCount} first20=`,
    ...
    debugLog(
      `LocalAIEngine: AudioBuffer length=${buffer.length}, duration=${buffer.duration}, sampleRate=${buffer.sampleRate}, channelMismatch=${mismatch}, first5=`,
      Array.from(channel.slice(0, 5)),
    );
    return buffer;
  }
  ...
  private async startBuffer(
    ...
    debugLog(
      'LocalAIEngine: source.start(0) at currentTime=',
      this.startTime,

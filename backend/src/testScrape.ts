...
async function runTest() {
  ...
  try {
    ...
    // Verify assertions
    if (ch.title !== 'Chapter 1: The Awakening') throw new Error('Chapter title parsing failed!');
    if (!ch.content.includes('First paragraph of text.')) throw new Error('Chapter content extraction failed!');

...
function cleanReaderPatch(input: unknown): Partial<IReaderSettings> {
	...
	if (typeof source.highlightMode === "string" && READER_HIGHLIGHT_MODES.includes(source.highlightMode as ReaderHighlightMode)) {
	...
	if (typeof source.autoScrollBehavior === "string" && READER_AUTOSCROLL_BEHAVIORS.includes(source.autoScrollBehavior as ReaderAutoScrollBehavior)) {
